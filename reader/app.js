...
const APPENDIX_BASE = '../dist/appendices';
...
function loadSettings() {
  try {
    const mode = localStorage.getItem('contexta-furigana-mode');
    ...
    localStorage.removeItem('contexta-kanji-exposures');

    // Load reading progress
    const completed = localStorage.getItem('contexta-completed-chapters');
    if (completed) {
      JSON.parse(completed).forEach(n => state.completedChapters.add(n));
    }
    const last = localStorage.getItem('contexta-last-chapter');
...
function saveSettings() {
  try {
    localStorage.setItem('contexta-furigana-mode', state.furiganaMode);
    localStorage.setItem('contexta-completed-chapters',
      JSON.stringify([...state.completedChapters]));
    if (state.lastChapter) {
      localStorage.setItem('contexta-last-chapter', String(state.lastChapter));
    }
  } catch (e) {
...
function renderJapaneseWithFurigana(sentence) {
  const text = sentence.text || sentence.japanese || '';
...
function playVisibleSentenceAudio() {
  const sentences = document.querySelectorAll('.panel-sentence');
  ...
  const playBtn = best.querySelector('.play-btn');
...
function renderLanding() {
  ...
  for (const num of state.chapterList) {
    ...
    // Use the first sentence's image as the cover
    const coverId = ch.sentences.length ? ch.sentences[0].id : null;
    const coverHtml = coverId
      ? `<div class="card-cover"><img src="${IMAGE_BASE}/${coverId}.png" alt="" loading="lazy"></div>`
      : '';
...
function renderReader(chapter) {
  ...
  // Scene
  document.getElementById('scene-location').textContent = chapter.scene.setting || chapter.scene.location || '';
  ...
  // Build panels: a sentence with imagePrompt starts a new panel;
  // sentences without imagePrompt join the current panel.
  const panels = [];
  for (const s of chapter.sentences) {
    if (s.imagePrompt || panels.length === 0) {
      panels.push({ sentences: [s] });
    } else {
      panels[panels.length - 1].sentences.push(s);
    }
  }
  ...
  for (const panel of panels) {
    ...
    panelEl.className = 'sentence-panel';
    panelEl.dataset.sentenceId = panel.sentences[0].id;

    // Panel image — from the first sentence (which has imagePrompt)
    const leadSentence = panel.sentences[0];
    const imgSrc = leadSentence.image || `${IMAGE_BASE}/${leadSentence.id}.png`;
    const promptText = leadSentence.imagePrompt || '';
    ...
    // Sentence lines within the panel
    const sentencesContainer = document.createElement('div');
    sentencesContainer.className = 'panel-sentences';

    for (const s of panel.sentences) {
      const sentEl = document.createElement('div');
      sentEl.className = 'panel-sentence' + (s.type === 'teaching' ? ' teaching' : '');
      sentEl.dataset.sentenceId = s.id;
      ...
      textEl.innerHTML = renderJapaneseWithFurigana(s);
    ...
    panelEl.appendChild(sentencesContainer);
...
function setupScrollProgress() {
  window.addEventListener('scroll', () => {
    ...
    const cards = document.querySelectorAll('.panel-sentence');
...
function scrollToSentence(direction) {
  ...
  const targetIdx = direction === 'next'
    ? Math.min(currentIdx + 1, sentences.length - 1)
    : Math.max(currentIdx - 1, 0);

  sentences[targetIdx].scrollIntoView({ behavior: 'smooth', block: 'center' });
}
